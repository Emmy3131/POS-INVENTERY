...
document.getElementById('transList').addEventListener('click', (e)=>{
    if (e.target.closest('#transDetails')) {
        ...
        _transactionViewJs.transactionDetailsView(transaction);
    }
});
...
const transactionDetailsView = (transaction)=>{
    ...
    if (!transaction || !transaction.items || transaction.items.length === 0) {
        container.insertAdjacentHTML("beforeend", `<div class="flex flex-col items-center justify-center py-12 text-center">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
      ...
      <div class="flex items-start justify-between mb-6">
        ...
        <div class="text-right">
          <p class="text-xl font-bold text-green-600">\u{20A6}${transaction.orderTotal?.toLocaleString() || '0'}</p>
          ...
          <div class="flex items-center p-6 hover:bg-gray-50 transition-colors duration-150">
            ...
            <div class="ml-6 flex-1 min-w-0">
              ...
              <div class="flex items-center mt-2 space-x-4">
                ...
                <span class="text-gray-500 text-sm">\u{2022}</span>
                <span class="text-gray-600 text-sm">Unit price: \u{20A6}${(item.price || 0).toLocaleString()}</span>
              </div>
            </div>
            <div class="text-right ml-4">
              <p class="font-bold text-gray-900">\u{20A6}${((item.price || 0) * (item.quantity || 1)).toLocaleString()}</p>
              <p class="text-sm text-gray-500 mt-1">${item.price || 0} \xd7 ${item.quantity || 1}</p>
            </div>
          </div>
        `).join('')}
      </div>
    ...
    <div class="bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl border border-gray-200 p-6 mt-6">
      <div class="flex justify-between items-center">
        <div>
          ...
          <p class="text-xl font-bold text-gray-900 mt-1">\u{20A6}${transaction.orderTotal?.toLocaleString() || '0'}</p>
        </div>
        <div class="text-right">
...
const transactionView = {
    render (transactions) {
        ...
        <tr id="${tr.id}" class="hover:bg-gray-50">
          ...
          <td class="px-6 py-4 text-gray-900 font-semibold">\u{20A6}${tr.orderTotal.toLocaleString()}</td>
